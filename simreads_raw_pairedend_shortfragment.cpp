...
pair<RawRead,RawRead> ReadSimulator::random_read_pair() const
{
   ...
   // cout << "H-length: " << haplotype_length << " config-length: " << config_.read_length << endl;
    ...
    size_t position1 = 0;
    size_t position2 = haplotype_length;
    ...
    if (position1 + config_.read_length <= haplotype_length) {
      sequence1 = hapref->full_sequence(h, position1, position1 + config_.read_length);
      sequence2 = hapref->full_sequence(h, position2 - config_.read_length,position2);

    } else {
      sequence1 = hapref->full_sequence(h, position1,haplotype_length);
      sequence2 = hapref->full_sequence(h, 0,position2);
    }
    ...
    string quality1(strlen(sequence1.c_str()), '\0');
    string quality2(strlen(sequence2.c_str()), '\0');
    for (size_t i=0; i<strlen(sequence1.c_str()); ++i) {
        quality1[i] = base_quality_distributions_[i](gen_); // [0,40]
    }
    for (size_t i=0; i<strlen(sequence2.c_str());++i) {
    ...
    read1.quality = string(strlen(quality1.c_str()), '\0');
    ...
    // standard SAM ASCII encoding (not Illumina)
    read2.quality = string(strlen(quality2.c_str()), '\0');
